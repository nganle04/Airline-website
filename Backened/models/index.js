"use strict";

const fs = require("fs");
const path = require("path");
const Sequelize = require("sequelize");
const basename = path.basename(__filename);
const env = process.env.NODE_ENV || "development";

const config = require(__dirname + "/../config/config.json")[env];
const db = {};

let sequelize;
if (config.use_env_variable) {
  sequelize = new Sequelize(process.env[config.use_env_variable], config);
} else {
  sequelize = new Sequelize(
    config.database,
    config.username,
    config.password,
    config
  );
}

fs.readdirSync(__dirname)
  .filter((file) => {
    return (
      file.indexOf(".") !== 0 && file !== basename && file.slice(-3) === ".js"
    );
  })
  .forEach((file) => {
    const model = require(path.join(__dirname, file))(
      sequelize,
      Sequelize.DataTypes
    );
    db[model.name] = model;
  });

Object.keys(db).forEach((modelName) => {
  if (db[modelName].associate) {
    db[modelName].associate(db);
  }
});

db.sequelize = sequelize;
db.Sequelize = Sequelize;
db.user = require("./user")(sequelize, Sequelize.DataTypes);
db.airport = require("./airport")(sequelize, Sequelize.DataTypes);
db.airline = require("./airline")(sequelize, Sequelize.DataTypes);
db.airplane = require("./airplane")(sequelize, Sequelize.DataTypes);
db.Baggage = require("./baggage")(sequelize, Sequelize.DataTypes);
db.boardingPass = require("./boardingpass")(sequelize, Sequelize.DataTypes);
db.passenger = require("./passenger")(sequelize, Sequelize.DataTypes);
db.ticket = require("./ticket")(sequelize, Sequelize.DataTypes);
db.crew = require("./crew")(sequelize, Sequelize.DataTypes);
db.flight = require("./flight")(sequelize, Sequelize.DataTypes);

// call assocaite method
db.crew.associate(db);
db.flight.associate(db);
db.boardingPass.associate(db);
db.ticket.associate(db);
db.airplane.associate(db);
db.passenger.associate(db);
db.airline.associate(db);


module.exports = db;
